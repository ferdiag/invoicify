...
describe("EditCustomerService", () => {
  ...
  it("returns id on success", async () => {
    ...
    const patch: Partial<CustomerInsertType> = { name: "ACME" };
    await expect(editCustomerService.execute(id, patch)).resolves.toEqual({
      id,
    });
  });
  ...
  it("maps not found to 500", async () => {
    ...
    const p = editCustomerService.execute("aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", patch);
  ...
  it("maps db errors to 500", async () => {
    ...
    const patch: Partial<CustomerInsertType> = { name: "X" };
    const p = editCustomerService.execute("11111111-2222-3333-4444-555555555555", patch);
