...
export async function authenticate(request: FastifyRequest, reply: FastifyReply) {
  ...
  if (!header) {
    reply.log.warn("Missing Authorization header");
    return reply.code(401).send({ error: "Unauthorized", message: "Missing Authorization header" });
  }

  const match = header.match(/^Bearer\s+(.+)$/i);
  if (!match) {
    reply.log.warn(`Invalid Authorization format: ${header.substring(0, 20)}...`);
    return reply.code(401).send({ error: "Unauthorized", message: "Invalid Authorization format" });
  }
  ...
  try {
    ...
    reply.log.warn(`Auth failed: ${msg}`);
